<?php $__env->startSection('title', 'Usuarios del Storage - ' . $storage->name . ' - Tcloud'); ?>

<?php $__env->startSection('content'); ?>
<div class="p-6" x-data="{
    userStorages: [],
    storageId: <?php echo e($storage->id); ?>,
    ...
    <div class="flex justify-between items-center mb-6">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-bold text-gray-800">Usuarios del Storage: <?php echo e($storage->name); ?></h1>
        </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                <template x-for="assignment in userStorages" :key="assignment.user_id">
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="assignment.user_email"></td>
    ...
    <div x-show="showAssignModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
        ...
        <div class="bg-white rounded-lg p-6 w-full max-w-md" @click.away="showAssignModal = false">
            <h2 class="text-xl font-bold mb-4">Asignar Usuario</h2>
            <form @submit.prevent="assignUser(new FormData($event.target))">
                <div class="mb-4">
        ...
        <div class="bg-white rounded-lg p-6 w-full max-w-md" @click.away="showEditModal = false">
            ...
            <template x-if="editingAssignment">
                <form @submit.prevent="updateAssignment(new FormData($event.target), editingAssignment.user_id)">
                    <div class="mb-4">
                        <label class="block text-sm font-medium mb-1">Usuario</label>
                        <span x-text="editingAssignment.user_email" class="text-gray-600"></span>
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium mb-1">Permisos</label>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/resources/views/admin/storage-users.blade.php ENDPATH**/ ?>