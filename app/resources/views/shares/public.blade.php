                ...
                <div class="p-6">
                    @if($file->is_folder)
                        ...
                        <div id="folder-contents" class="space-y-2">
                            ...
                            @foreach($folderContents as $item)
                                ...
                                @endphp
                                <div class="flex items-center justify-between bg-slate-50 hover:bg-slate-100 rounded-lg p-4 transition-colors {{ $item->is_folder ? 'cursor-pointer' : '' }}" @if($item->is_folder) onclick="window.location='{{ route('share.folder', ['token' => $share->token, 'folder_id' => $item->id]) }}'" @endif data-file-id="{{ $item->id }}">
                                    <div class="flex items-center gap-3">
                                        @if($item->is_folder)
                                            ...
                                            <div>
                                                <span class="font-medium text-slate-700 hover:text-blue-600">{{ $item->name }}</span>
                                    ...
                                    <div class="flex items-center gap-2">
                                        @if(!$item->is_folder && $isPreviewable)
                                            @php $currentIndex = $previewableIndex++; @endphp
                                            <button @click="openPreview({{ $currentIndex }})" class="p-2 bg-green-100 hover:bg-green-200 text-green-600 rounded-lg transition-colors" title="Ver">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        @endif
                                        @if(!$item->is_folder)
                                            <a href="{{ route('share.file-download', ['token' => $share->token, 'file_id' => $item->id]) }}" class="p-2 bg-blue-100 hover:bg-blue-200 text-blue-600 rounded-lg transition-colors" title="Descargar">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        @if(in_array($share->permissions, ['write', 'full']) && !$item->is_folder)
                                            <form action="{{ route('share.rename', ['token' => $share->token, 'file_id' => $item->id]) }}" method="POST" class="flex items-center gap-1" @submit.prevent="renameFile($event, {{ $item->id }})">
                                                @csrf
                                                <input type="text" name="name" value="{{ $item->name }}" class="border border-slate-300 px-2 py-1 rounded text-sm w-32">
                                            ...
                                            <button @click="confirmDelete('{{ route('share.delete', ['token' => $share->token, 'file_id' => $item->id]) }}', {{ $item->id }}, '{{ $item->name }}')" type="button" class="p-2 bg-red-100 hover:bg-red-200 text-red-600 rounded-lg transition-colors" title="Eliminar">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @else
                        ...
                        <div class="flex flex-wrap gap-3 justify-center">
                            <a href="{{ $fileUrl }}" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-medium transition-colors">
