# Spec: share-upload (propuesta — entra en vigor tras archivar el change)

> Esta spec define el contrato de comportamiento de la subida por link público.
> Hoy no existe como spec en `openspec/specs/`; se crea cuando el change `2026-07-23-cloud-nginx-upload-body-size` se archive.
...
- `client_max_body_size` de Nginx en el vhost `cloud.mediaserver.com.co` debe estar alineado con el tamaño máximo permitido por la app. Hoy `10G`. Cambio en `2026-07-23-cloud-nginx-upload-body-size`.
