...
- **THEN** existe una fila en `watermark_audit_log` con `actor_user_id=5`, `action='rewind_pair'`, `keyword_id=42`, `storage_id=7`, `before_value=<timestamp actual>`, `after_value=NULL`, `metadata={"reason":"admin-trigger", "ip":"...","user_agent":"..."}`
...
La tabla SHALL tener un índice compuesto `(keyword_id, storage_id, created_at DESC)` para responder "¿quién movió este par?" y otro `(actor_user_id, created_at DESC)` para "¿qué acciones hizo este usuario?". Las consultas de auditoría SHALL usar estos índices y SHALL completarse en < 100 ms sobre 1M de filas (validado con EXPLAIN ANALYZE en staging).
