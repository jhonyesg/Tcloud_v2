...
### Requirement: Cálculo de prioridad reusa ScanAndSubmitCommand
El sistema SHALL calcular la prioridad de cada job encolado con `ConvertAndTranscribeJob::calculatePriority($storage->transcription_priority ?? 0, true, false)` — exactamente la misma fórmula usada por `app/app/Console/Commands/ScanAndSubmitCommand.php:70-74`, garantizando que el admin-triggered bulk no tenga prioridad distinta al scheduler-triggered bulk.

#### Scenario: Storage con prioridad 10 produce prioridad de job alta
- **WHEN** la fila apunta a un `StorageProvider` con `transcription_priority = 10` y el usuario la encola vía bulk dispatch
- **THEN** el `ConvertAndTranscribeJob::dispatchWithPriority($fileId, true, $priority)` se invoca con `$priority = calculatePriority(10, true, false)`, que usando la fórmula existente produce un valor alto que va a la cola `transcription-high` (no `medium` ni `low`)

#### Scenario: Storage con prioridad 0 produce prioridad baja
- **WHEN** la fila apunta a un `StorageProvider` con `transcription_priority = 0` o `null`
- **THEN** el job se encola en la cola `transcription-low`, respetando la no-urgencia del storage
...
- **WHEN** Redis no responde y el primer `dispatchWithPriority()` lanza `Predis\Connection\ConnectionException`
...
El sistema SHALL envolver cada llamada individual a `ConvertAndTranscribeJob::dispatchWithPriority()` en try/catch. Si una falla, incrementará `errors` y continuará con la siguiente sin abortar el batch entero. Cada error individual se loguea como `warning` con `Log::warning()`.
