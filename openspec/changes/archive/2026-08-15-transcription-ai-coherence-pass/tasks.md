...
- [x] Crear `app/app/Services/Ia/TranscriptionCoherencePass.php`:
...
- [x] Agregar al schema de `TranscriptorSettings`:
...
- [x] En `persistSegmentsAndUpdate`, después de `applyToSegments`:
...
- [x] Con un segmento con spanglish, el `text` queda en español coherente sin rastro de inglés.
- [x] `text_raw` conserva el original del transcriptor.
- [x] Segmentos sin inglés residual no se envían al LLM.
- [x] Con > `ai_coherence_max_segments` flagged, solo se corrigen los primeros N.
- [x] Si el LLM falla, la transcripción se guarda igual (state=done) con el texto del diccionario.

## 5. Archivar

- [x] Mover a `archive/2026-08-15-2026-08-15-transcription-ai-coherence-pass/`.
