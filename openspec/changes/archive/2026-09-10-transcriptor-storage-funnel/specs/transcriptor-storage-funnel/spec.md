...
El sistema SHALL renderizar, en la tabla de la pestaña Storages del módulo API Transcriptor (`/ia/api-transcriptor`), tres columnas adicionales por fila con conteos agregados desde la tabla `transcriptions`:

- **Pendientes** = filas `transcriptions` con `state IN ('pending', 'error')` AND `dispatched_at IS NULL`.
- **En curso** = filas `transcriptions` con `state IN ('queued', 'processing')` OR (`dispatched_at IS NOT NULL` AND `finished_at IS NULL`).
- **Listos** = filas `transcriptions` con `state = 'done'`.

Los conteos SHALL agruparse por `files.storage_provider_id` mediante una sola query agregada. Cada fila SHALL mostrar los tres números separados visualmente, alineados a la derecha, sin texto adicional cuando el conteo sea cero.
...
- **THEN** la fila correspondiente muestra `Pendientes: 8`, `En curso: 2`, `Listos: 430`
...
- **THEN** la fila muestra `0` en las tres columnas, alineado como los demás números, sin texto "Sin trabajos" ni placeholders

#### Scenario: Conteos erróneos o muertos cuentan como Pendientes
- **WHEN** un storage tiene 4 filas en `state = 'error'` con `dispatched_at IS NULL`
- **THEN** esas 4 filas se contabilizan en la columna Pendientes (junto con las `pending`), porque仍未 no han sido despachadas o reintentadas
