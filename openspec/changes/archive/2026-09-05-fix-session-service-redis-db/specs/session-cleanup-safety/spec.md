...
- **WHEN** `cleanOrphans()` escanea N filas y borra M, donde `M/N <= 0.5`
...
El job `sessions:cleanup` SHALL soportar un parámetro `dry-run` (vía variable de entorno o flag CLI) que permite ejecutar la lógica de detección sin borrar ninguna fila, devolviendo cuántas habría borrado. Esto permite al admin auditar el efecto del cron antes de activarlo en producción.

#### Scenario: Admin corre dry-run desde consola
- **WHEN** se ejecuta `php artisan tinker --execute="app(SessionService::class)->cleanOrphans(dryRun: true)"`
...
- **THEN** SHALL existir una entrada en `laravel.log` con `channels=production` y `message=sessions.cleanup.completed`
...
#### Scenario: Admin sube el intervalo a 60 minutos
- **WHEN** `system_settings.sessions_cleanup_interval_minutes = 60`
- **THEN** el cron SHALL correr cada 60 minutos
- **AND** SHALL NO loguear warning

#### Scenario: Admin baja el intervalo a 1 minuto (peligroso)
- **WHEN** `system_settings.sessions_cleanup_interval_minutes = 1`
- **THEN** el cron SHALL correr cada 1 minuto
- **AND** SHALL loguear `sessions.cleanup.interval_too_aggressive` con el valor detectado
