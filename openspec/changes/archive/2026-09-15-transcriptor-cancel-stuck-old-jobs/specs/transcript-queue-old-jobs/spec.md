# transcriptor-cancel-stuck-old-jobs Specification
...
## Comandos relacionados (sin cambios)

- `transcription:tick` (cada 2 min en scheduler) — sigue filtrando por hoy. No se toca.
- `transcription:poll-results` (cada 1 min en scheduler) — sigue re-enviando solo `pending sin job_id` del día actual. No se toca.
- `transcription:purge-stale-pending` (CLI manual) — sigue purgando solo filas `pending` sin `job_id`. No se solapa.
- `transcription:retry-batch-upstream` (semanal Lun 04:00) — sigue re-enviando `error|dead` de las últimas 168h. No se solapa.

## Aclaraciones operativas

- **`--days` default = 7** (mismo horizonte que `retry-batch-upstream`).
- **`--min-age-minutes` default = 60** (protección contra jobs recién enviados que el upstream aún no registró).
- **`--max-ratio` default = 0.5** (mismo patrón que `purge-stale-pending`).
- **`--stagger-ms` default = 750** (entre cada cancelación individual, no entre chunks). Con 750 ms × 3.092 = ~40 min totales.
- **`--batch` default = 100** (tamaño del chunk para paginación interna).
- **`--apply` debe ser explícito**. `--dry-run` es el comportamiento por defecto si se omite `--apply`.
- **`--actor-id` opcional** (default null = CLI manual). Si se integra con la UI en el futuro, este sería el `session('user_id')`.
