...
- **THEN** a gzip-compressed SQL dump file is created in `backup/db/` with the naming pattern `tcloudstorage_YYYY-MM-DD_HH-MM-SS.sql.gz`
...
- **THEN** all files in `backup/db/` matching `tcloudstorage_*.sql.gz` with a modification time older than 20 days SHALL be deleted
...
- **WHEN** the backup script runs and `backup/db/` does not exist
- **THEN** the script SHALL create the `backup/db/` directory before writing the backup
...
- **THEN** a new backup file SHALL be created immediately in `backup/db/`
