# Spec: transcription-virtual-scope

## Purpose

Define la herencia virtual de transcripciones entre storages padre e hijos: un storage con `transcription_enabled=true` que contiene subpaths pertenecientes a otros storages hijos también habilitados expone, como capa de solo lectura, los archivos y transcripciones de sus descendientes en las consultas de la UI/API, sin duplicar trabajo de transcripción ni filas en `transcriptions`. La herencia es una vista agregada que se resuelve en cada request, no una réplica.
