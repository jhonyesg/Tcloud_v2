...
Define the frontend behavior for keeping the user authenticated across long-running sessions and for gracefully handling session expiry without forcing a hard reload, including the keep-alive ping, the session-expired intercept flow, the `apiFetch` wrapper, the backend `POST /auth/ping` endpoint, and the `X-Session-Expired: 1` marker emitted by the `Authenticate` middleware.
